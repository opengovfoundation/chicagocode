...
def parseParagraphHeader(header):
    ...
    m = re.match( ur'(?um)^(\s*)(?:\u00A7)?\s*[^\d]?((?:\d+-)*\d+)(\.(?:\d+.)*\d+)?\s*(.*)\s*$', header )
    if not m:
...
class Title:
    ...
    def parse(self):
        ...
        m = re.match( ur'(?mu)^\s*TITLE\s+(\d+)\s+(.+)\s*$', self.content)
        #print "m = ",m
        if m:
            self.titleId = m.group(1)
...
class Chapter:
    ...
    def parse(self):
        ...
        m = re.match( ur'(?mu)^\s*(?:CHAPTER|Ch\.)\s+(\d+-\d+)\s+(.+)\s*$', self.content)
        #print "m = ",m
        if m:
...
class Law(SectionContainer):
    ...
    def writeXML(self, dirname):
        self.parseItems()
        filename = os.path.join( dirname, '%s.xml' % ( self.getId() ) )
        indentStep = "     "
        indent = indentStep
        indents = []
        ...
        ofile.write('%s<order_by>%s</order_by>\n' % (indent, self.makeOrderByLaw(self.getId())) )
        ofile.write('%s<text>\n' % (indent) )
        indents.append(indent)
        indent += indentStep
        ...
        indent = indents.pop()
        ofile.write('%s</text>\n' % (indent) )
        history = self.historyMap.get(self.getId(), self.history)
        if history is None:
            history = ""
    ...
    def parse(self):
        ...
        m = re.match( ur'(?um)^(\s*)(?:\00A7)?\s*[^\d]?\D?((?:\d+-)*\d+)(\.(?:\d+.)*\d+)?\s*(.*)\s*$', self.name )
...
class TitleParser(HTMLParser):
    ...
    CHAPTER_COLOR = "#008f00"

    LAW_COLOR = "#011993"

    def __init__(self, historyMap):
        HTMLParser.__init__(self)
        self.historyMap = historyMap
    ...
    def haveChapterTitle(self):
        ...
        if color == self.CHAPTER_COLOR:
            #print "haveChapterTitle: Returning true"
            return True
        else:
    ...
    def haveLawTitle(self):
        ...
        sclass = p.attributes.get('class')
        color = self.colors.get(sclass)
        if color == self.LAW_COLOR:
            m = re.match( ur'(?umi)^\s*appendix.*$', p.content)
            if m:
                return False
            return True
        else:
    ...
    def parseParagraph(self):
        ...
        if (self.state == 'BODY') and (p.attributes.get('class') == 'p1'):
            ...
            self.state = 'TITLE'
        elif (self.state in ['TITLE', 'ARTICLE', 'LAW']) and self.haveChapterTitle():
        ...
        #     self.state = 'ARTICLE'
        elif ((self.state in ['ARTICLE', 'CHAPTER', 'LAW']) and 
            ...
            self.state = 'LAW'
        elif (self.state == 'LAW') and ((p.attributes.get('class') == 'p2') or
...
def processDir(dirname, historyMap):
    ...
    files.sort()
    for fname in files:
        if re.match( ur'title.*\.html', fname ):
...
def processTables(dirname):
    ...
    if not os.path.exists(path):
        raise Exception("processTables: Table file: %s not found" % (path))
    parser = TableParser()
    ifile = file( path )
    doc = unicode(ifile.read(), 'utf-8')
    ifile.close()
